...
- **`Builder.where()`/`.where_in()`/`.where_not_in()` are not translated**
  into Meilisearch's `filter` syntax.
- **Soft-deleted records are removed from the index entirely**, not tagged
  ...
  `database` engine.
- **`update_index_settings` is not implemented** for this driver yet, even
  though Meilisearch has a real settings API (searchable attributes,
  ranking rules) — `sync-index-settings` will raise
  `IndexSettingsNotSupportedError` until that lands.
