...
def test_search_matches_query_across_all_fields(
...
def test_search_excludes_soft_deleted_by_default(
    adapter: FakeAdapter, articles: list[Article]
) -> None:
    engine = CollectionEngine()
...
def test_with_trashed_includes_soft_deleted(
    ...
    engine = CollectionEngine()
    builder = Builder(
        Article, "archived", engine=engine, adapter=adapter
    ).with_trashed()
    assert [a.id for a in engine.get(builder)] == [3]


def test_only_trashed_returns_only_soft_deleted(
    adapter: FakeAdapter, articles: list[Article]
) -> None:
    engine = CollectionEngine()
...
def test_where_constrains_results(
    adapter: FakeAdapter, articles: list[Article]
) -> None:
    engine = CollectionEngine()
...
def test_where_in_constrains_results(
    adapter: FakeAdapter, articles: list[Article]
) -> None:
    engine = CollectionEngine()
...
def test_where_not_in_excludes_results(
    adapter: FakeAdapter, articles: list[Article]
) -> None:
    engine = CollectionEngine()
...
def test_paginate_returns_page_metadata(
    adapter: FakeAdapter, articles: list[Article]
) -> None:
    engine = CollectionEngine()
